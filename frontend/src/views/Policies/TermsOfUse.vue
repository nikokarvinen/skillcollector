...
<template>
  <div class="modal" @click="$emit('close')" tabindex="0">
    <div class="modal-content" @click.stop>
      <div tabindex="0" @keydown.esc="$emit('close')">
        ...
        <section @click.stop>
          ...
          <h2>{{ t("limitation_title") }}</h2>
          <p>{{ t("limitation_body") }}</p>
          <h2>{{ t("changes_title") }}</h2>
          <p>{{ t("changes_body") }}</p>
          <h2>{{ t("governing_title") }}</h2>
          <p>{{ t("governing_body") }}</p>
          <h2>{{ t("contact_title") }}</h2>
          <p>{{ t("contact_body") }}</p>
          <button class="close-btn" @click="$emit('close')">{{ t("close_button") }}</button>
        </section>
      </div>
    </div>
  </div>
